    ...
    axios
      ...
      .then((response) => {
        if (response && response.status === 200) {
          ...
          console.log(
            `loginServices: status=${response?.status}, data=`,
            response?.data
          );
          reject(
            new Error(`El inicio de sesion ha fallado, ${response?.message}`)
          );
        }
      })
      .catch((error) => {
    ...
    axios
      ...
      .then((response) => {
        if (response && response.status === 200) {
          ...
          reject(new Error(`El inicio de sesion con token ha fallado, ${response?.message}`));
        }
      })
      .catch((error) => {
        console.log({ error });
        reject(new Error(`El inicio de sesion con token ha fallado, ${error?.message}`));
      });
  });
}
